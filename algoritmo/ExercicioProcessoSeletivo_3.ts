...
export function calculateSimpleExpression(simpleExpression: SimpleExpression): OptionalNumber {
    const { operation, lhs, rhs } = simpleExpression;
...
export function calculateOperand(operand: Operand): OptionalNumber {
    if (operand === null || typeof operand === 'number') {
        ...
        return calculate(operand); // recursivamente calcula subexpressão
    }
}

// 1 ponto
export function calculate(expression: Expression): OptionalNumber {
    const lhs = calculateOperand(expression.lhs);
    const rhs = calculateOperand(expression.rhs);
    return calculateSimpleExpression({ operation: expression.operation, lhs, rhs });
}
